...
struct Rect {
	...
	Point Center() const { return Point(point.x + (int)(size.width / 2), point.y + (int)(size.height / 2)); }

	bool IsEmpty() const { return size.IsEmpty(); }
	uint Area() const { return size.Area(); }

	bool Contains(const Point& point) const { return point >= this->point && point < RightBottom(); }
