...
public:
	...
	constexpr uint AsUnsigned() const {
		if (value < 0) { throw std::invalid_argument("invalid unsigned value"); }
		return value;
	}
};
...
public:
	...
	constexpr ValueTag(ValuePixel value) : ValueTag(value.value, Tag::Pixel) {}
