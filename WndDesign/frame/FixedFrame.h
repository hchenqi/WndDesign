...
public:
	FixedFrame(uint height, child_ptr<Auto, Assigned> child) : WndFrame(std::move(child)), size(0, height) {
		size.width = UpdateChildSizeRef(this->child, size).width; SetChildData<uint>(this->child, child_auto_assigned);
	}
	FixedFrame(uint width, child_ptr<Assigned, Auto> child) : WndFrame(std::move(child)), size(width, 0) {
		size.height = UpdateChildSizeRef(this->child, size).height; SetChildData<uint>(this->child, child_assigned_auto);
	}
	FixedFrame(Size size, child_ptr<Assigned, Assigned> child) : WndFrame(std::move(child)), size(size) {
		UpdateChildSizeRef(this->child, size); SetChildData<uint>(this->child, child_assigned_assigned);
	}
private:
	Size size;
private:
