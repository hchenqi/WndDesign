...
private:
	virtual const Size OnSizeRefUpdate(Size size_ref) override {
		Size child_size = UpdateChildSizeRef(child, size_ref); child_height = child_size.height; return child_size; 
	}
	virtual void OnChildSizeUpdate(const WndObject& child, Size child_size) override { child_height = child_size.height; }
private:
